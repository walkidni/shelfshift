...
def build_parser() -> argparse.ArgumentParser:
    ...
    detect.set_defaults(
        func=lambda args: (
            _cmd_detect_csv(args) if Path(args.input).exists() else _cmd_detect_url(args)
        )
    )

    import_url_cmd = subparsers.add_parser(
        "import-url", help="Import canonical products from one or more URLs"
    )
    import_url_cmd.add_argument("url", nargs="+", help="One or more product URLs")
    import_url_cmd.add_argument("--strict", action="store_true")
    import_url_cmd.set_defaults(
        func=lambda args: _cmd_import_url(
            argparse.Namespace(
                **{**vars(args), "url": args.url if len(args.url) > 1 else args.url[0]}
            )
        )
    )
    ...
    convert.add_argument(
        "--to",
        required=True,
        choices=["shopify", "bigcommerce", "wix", "squarespace", "woocommerce"],
    )
    ...
    validate_cmd = subparsers.add_parser(
        "validate", help="Validate canonicalized products imported from source CSV"
    )
    ...
    import_csv_cmd = subparsers.add_parser(
        "import-csv", help="Import canonical products from source CSV"
    )
    ...
    export_csv_cmd = subparsers.add_parser(
        "export-csv", help="Export canonical JSON payload to target CSV"
    )
    export_csv_cmd.add_argument("input", help="Canonical product JSON path")
    export_csv_cmd.add_argument(
        "--to",
        required=True,
        choices=["shopify", "bigcommerce", "wix", "squarespace", "woocommerce"],
    )
