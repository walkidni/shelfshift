...
_FIXTURES_ROOT = Path(__file__).resolve().parents[1] / "fixtures" / "exporter" / "shopify"


def test_shopify_csv_matches_golden_fixture_two_variants() -> None:
    fixture_path = _FIXTURES_ROOT / "shopify_one_product_two_variants_full.csv"
    ...
    product.variants[0].price = Price(current=Money(amount=24.99, currency="USD"))
    product.variants[1].price = Price(current=Money(amount=24.99, currency="USD"))
...
def test_shopify_csv_matches_golden_fixture_simple_product() -> None:
    fixture_path = _FIXTURES_ROOT / "shopify_one_simple_product_full.csv"
    expected = read_fixture_frame(fixture_path)
    assert list(expected.columns) == SHOPIFY_COLUMNS
